...
class CFGKT(nn.Module):
    def __init__(self, n_concepts, n_pid, d_model, n_blocks,
        ...
        self.trans = Architecture(n_question=self.n_question, n_blocks=n_blocks, n_heads=n_heads,
        ...
        # initializing a matrix to response_embed the underlying latent knowledge concepts.
        self.MS = nn.Parameter(torch.Tensor(memory_size, self.d_model))
        kaiming_normal_(self.MS)

        self.state_attn = state_fusion(self.d_model, n_heads)
        if self.attn_ == True:
            self.final_attn = nn.MultiheadAttention(embed_dim=self.d_model, num_heads=4, dropout=dropout,
                                                    batch_first=True)
...
class state_fusion(nn.Module):
    def __init__(self, hid_dim, n_heads):
        ...
        self.attn_distilling = distilling(hid_dim)
    ...
    def forward(self, query, key, value):
        bsz = query.shape[0]
        device = query.device
        Q = self.w_q(query).to(device)
        K = self.w_k(key).to(device)
        V = self.w_v(value).to(device)
...
class exp_acq(nn.Module):
    ...
    def forward(self, q_embed_data, qa_embed_data):
        ...
        exp = self.exp.repeat(q_embed_data.size(0), 1).to(q_embed_data.device)
        h = list()
