...
class CsKTNP(BaseKt):
    def __init__(self, skill_num, emb_size=128, attention_blocks=4, dropout=0.1,
                 d_ff=256, kq_same=True, final_fc_dim=256, attn_heads=8, separate_qa=False, l2=1e-5,
                 r=1.0, gamma=1.0, **kwargs):
        super().__init__("CsKT-NP")
        """
        Input:
        ...
        if self.n_pid > 0:
            ...
            # else:
            #     self.difficult_param = nn.Embedding(self.n_pid, embed_l)  #
            self.q_embed_diff = nn.Embedding(self.n_question , embed_l)  #
            self.qa_embed_diff = nn.Embedding(2 * self.n_question, embed_l)  #
    ...
    def forward(self, x, mask=None, training=None, **kwargs):
        ...
        #                q_embed_diff_data  # # + uq *(h_rt+d_ct) # （q-response emb diff + question emb diff）
...
class Architecture(nn.Module):
    def __init__(self,  n_blocks, d_model, d_ff, n_heads, dropout, kq_same,  r, gamma):
        super().__init__()
        """
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask, zero_pad):
        ...
        scores = attention(q, k, v, mask, self.dropout, zero_pad, self.r, self.gamma, self.kernel_bias)
...
def manual_cdist(x1, x2, p=2):
    ...
    # 计算点积
    x1_flat = x1.reshape(-1, x1.shape[-2], x1.shape[-1])
    x2_flat = x1.reshape(-1, x2.shape[-2], x2.shape[-1])

    # 矩阵乘法计算点积
    dot_product = torch.bmm(x1_flat, x2_flat.transpose(-2, -1))

    # 计算距离矩阵：sqrt(x^2 + y^2 - 2xy)
    distance_sq = x1_norm.reshape(dot_product.shape[:-1] + (1,)) + x2_norm.reshape(
        dot_product.shape[:-1] + (1,)).transpose(-2, -1) - 2 * dot_product

    # 处理数值误差，确保非负
    distance_sq = torch.clamp(distance_sq, min=1e-10)

    if p == 2:
        distance = torch.sqrt(distance_sq)
    else:
        distance = distance_sq ** (1 / p)

    # 恢复原始shape
    final_shape = x1.shape[:-2] + (x1.shape[-2], x2.shape[-2])
    return distance.view(final_shape)
def coneattn(q, k, r=1, gamma=1):
    q_x, q_y = map_xi(q)
    k_x, k_y = map_xi(k)
    q_y = q_y.unsqueeze(3)
    k_y = k_y.unsqueeze(2)
    if str(q.device).startswith('npu'):
        out = torch.maximum(torch.maximum(q_y, k_y),
                            (manual_cdist(q_x, k_x) / torch.sinh(torch.tensor(r)) +
                             torch.add(q_y, k_y)) / 2)
    else:
        out = torch.maximum(torch.maximum(q_y, k_y),
...
def attention(q, k, v,  mask, dropout, zero_pad, r, gamma, kernel_bias):
    """
