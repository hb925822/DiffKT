...
def attention(
    ...
    # scorted_attention
    BS, H, L, _ = scores.shape
    N = BS * H * L
    scores_flat = scores.view(N, L)  # (N, L)

    # 降序排序
    sorted_vals, sorted_idx = torch.sort(scores_flat, descending=True, dim=1)  # (N, L)

    # 计算累积和
    cumsum = torch.cumsum(sorted_vals, dim=1)  # (N, L)

    # 找到每行第一个 cumsum >= threshold 的位置
    # 创建布尔掩码
    exceeds = cumsum >= sparse_ratio  # (N, L), bool

    # 处理极端情况：如果所有值加起来都 < threshold（理论上不会，因为 sum=1）
    # 但为了安全，确保至少有一个 True
    exceeds[:, -1] = True  # 强制最后一列为 True

    # 找第一个 True 的索引（即最小的 n 使得 cumsum >= threshold）
    first_exceed_idx = exceeds.long().argmax(dim=1, keepdim=True)  # (N, 1)

    # 构建保留 mask（在排序空间）：保留 [0, first_exceed_idx] 的所有位置
    range_idx = torch.arange(L, device=scores.device).unsqueeze(0)  # (1, L)
    keep_mask_sorted = range_idx <= first_exceed_idx  # (N, L), bool

    # 将 mask 映射回原始顺序
    keep_mask_original = torch.zeros_like(scores_flat, dtype=torch.bool)  # (N, L)
    keep_mask_original.scatter_(1, sorted_idx, keep_mask_sorted)

    # 应用 mask
    sparse_flat = scores_flat * keep_mask_original.float()

    # 重新归一化（重要！否则总和 < 1）
    sparse_flat = sparse_flat / sparse_flat.sum(dim=1, keepdim=True).clamp(min=1e-8)

    scores= sparse_flat.view(BS, H, L, L)
