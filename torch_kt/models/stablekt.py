...
'''
@inproceedings{liEnhancingLengthGeneralization2024,
...
class StableKT(BaseKt):
    def __init__(self, skill_num, problem_num,
...
class Architecture(nn.Module):
    def __init__(self, n_blocks, d_model,
                 d_ff, n_heads, dropout, r, gamma, kq_same=True):
        super().__init__()
        """
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask, zero_pad):
        ...
        q = q.transpose(1, 2)
        v = v.transpose(1, 2)
        # self.alibi=self.alibi.to(q.device)
        ...
        scores_hakt = attention_hakt(q[:, half_h:, :, :], k[:, half_h:, :, :], v[:, half_h:, :, :], self.d_k,
                                     mask, self.dropout, zero_pad, r=self.r,
                                     gamma=self.gamma)
...
def penumbral(q, k, r=1, gamma=0.1, eps=1e-6):
    ...
    if str(q.device).startswith('npu'):
        pairwise_dist = manual_cdist(q_x, k_x)
    else:
...
def attention_hakt(q, k, v, d_k, mask, dropout, zero_pad, r, gamma):
    """
    This is called by Multi-head atention object to find the values.
    """
...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    *We use rotary position embeddings in self-attention layers.
    ...
    def forward(self, x: torch.Tensor):
        """
        * `x` is the Tensor at the head of a key or a query with shape `[ batch_size, max_len, n_heads, d]`
        """
        # Extract the shape
